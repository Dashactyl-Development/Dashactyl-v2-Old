...
module.exports.run = async (userdb) => {
  ...
  router.get("/callback", async (req, res) => {
    ...
    let json = await fetch(
      'https://discord.com/api/oauth2/token',
      {
        ...
        body: {
          ...
          "code": req.query.code,
          "redirect_uri": encodeURIComponent(settings.discord.oauth2.link + settings.discord.oauth2.callbackpath)
        },
    ...
    console.log(json)
    if (json.ok === true) {
      let codeinfo = JSON.parse(await json.json());
      ...
      let userinfo = JSON.parse(await userinfo_raw.json());
